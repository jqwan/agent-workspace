...
- 任务 CRUD、搜索、筛选、排序、截止时间、颜色标签和状态统计
- 状态为未完成、已完成、已废弃；历史待办/处理中状态及未知状态一次性迁移为未完成
- 软删除任务，移入回收站后可恢复，也可手动永久删除
- 每个任务可有多个 pi 子会话，可切换、重命名和删除；会话可以全部删除，之后可重新创建。任务目录始终可编辑，仅作为新会话默认目录，已有会话沿用 session 中的工作目录
