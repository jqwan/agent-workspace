...
function syncTaskToolbarTitle() {
  ...
  const newTaskButton = $('#sidebar-new-task');
  const newNoteButton = $('#sidebar-new-note');
  newTaskButton.title = '新建任务';
  newNoteButton.title = '新建便签';
}
...
function renderPinnedNotes() {
  ...
  $('#topbar-note-buttons').innerHTML = `${topNotes.map((note) => `<button type="button" class="note-button color-${taskColor(note)}${customColors[taskColor(note)] ? ' custom-color' : ''}"${customColorStyle(taskColor(note))} data-topbar-note="${esc(note.id)}" data-note-id="${esc(note.id)}" data-note-placement="topbar" aria-label="${esc(noteLabel(note))}" title="${esc(noteLabel(note))}">${esc(noteInitial(note))}</button>`).join('')}<section id="topbar-note-panel" class="session-description-panel topbar-note-panel hidden" aria-label="便签详情"></section>`;
  renderMarquee();
}
function renderSessionNoteButtons() {
  ...
  root.innerHTML = notes.map((note) => `<button type="button" class="note-button session-note-button color-${taskColor(note)}${customColors[taskColor(note)] ? ' custom-color' : ''}"${customColorStyle(taskColor(note))} data-session-note="${esc(note.id)}" data-note-id="${esc(note.id)}" data-note-placement="session" aria-label="${esc(noteLabel(note))}" aria-haspopup="menu" aria-expanded="false" title="选择发送方式">${esc(noteInitial(note))}</button>`).join('');
}
...
function renderTaskSidebar() {
  ...
  const board = (type, label, icon, count, active) => `<button type="button" class="task-group-item${active ? ' active' : ''}" data-board-type="${type}" aria-label="${label}" aria-pressed="${active}"><span class="group-icon" aria-hidden="true">${icon}</span><span class="group-label">${label}</span><b>${number(count)}</b></button>`;
  const archiveActive = state.status === 'archived';
  const archive = `<button type="button" class="task-group-item${archiveActive ? ' active' : ''}" data-archive-group aria-label="回收站" aria-pressed="${archiveActive}"><span class="group-icon" aria-hidden="true">${GROUP_ICONS.archived}</span><span class="group-label">回收站</span><b>${number(archivedTasks.length + archivedNotes.length)}</b></button>`;
  $('#task-groups').innerHTML = board('tasks', '任务', '▦', activeTasks.length, state.status !== 'archived' && state.boardType === 'tasks') + board('notes', '便签', '▤', activeNotes.length, state.status !== 'archived' && state.boardType === 'notes') + archive;
}
...
function noteCard(note, compact = false) {
  ...
  const archiveInfo = note.status === 'archived' ? `<div class="archive-info">废弃${note.archivedAt ? ` · ${time(note.archivedAt)}` : ''}${note.purgeAt ? ` · 预计 ${time(note.purgeAt)} 自动删除` : ''}</div>` : '';
...
function renderNotesList() {
  ...
  const board = notes.length ? groups.map((group) => `<section class="task-board-column"><header class="task-board-head"><span class="badge board-group-badge" title="${esc(group.label)}">${esc(group.label)}</span><b>${number(group.items.length)}</b></header><div class="task-board-list">${group.items.map((note) => noteCard(note, true)).join('')}</div></section>`).join('') : empty;
  $('#task-list').innerHTML = `<div class="task-board${state.boardCardLayout === 'compact' ? ' compact-card-layout' : ''}">${board}</div>`;
...
function renderArchivedList() {
  ...
  const board = items.length ? groups.map((group) => `<section class="task-board-column"><header class="task-board-head"><span class="badge board-group-badge">${esc(group.label)}</span><b>${number(group.items.length)}</b></header><div class="task-board-list">${group.items.map((item) => item.kind === 'task' ? card(item.value, true) : noteCard(item.value, true)).join('')}</div></section>`).join('') : '<div class="task-board-empty"><strong>回收站为空</strong></div>';
  $('#task-list').innerHTML = `<div class="task-board${state.boardCardLayout === 'compact' ? ' compact-card-layout' : ''}">${board}</div>`;
  syncMasonryColumns($('#task-list')); syncOverflowTooltips($('#task-list'));
}
...
function openNoteForm(note = null, options = {}) {
  ...
  const renderPicker = () => {
    ...
    colorPicker.innerHTML = `<button type="button" class="color-add" data-add-note-color title="新增颜色" aria-label="新增颜色">＋</button>${options}`;
  };
  ...
  customInput.onchange = () => {
    ...
    const found = Object.entries(colorCatalog()).find(([, item]) => item.value === value);
    const key = found?.[0] || `custom-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
    if (!found) {
...
$('#sidebar-new-task').onclick = () => openNewBoardItem('tasks');
...
$('#session-sidebar-new-note').onclick = () => openNoteForm(null, { pinnedToSessionBar: true });
...
function closeTopbarNotePanel() { $('#topbar-note-panel')?.classList.add('hidden'); }
...
function openSessionNoteMenu(button, note) {
  ...
  menu.innerHTML = `<div class="session-note-menu-title">${esc(note.title?.trim() || '便签')}</div><button type="button" role="menuitem" data-session-note-mode="current">发送到当前会话</button><button type="button" role="menuitem" data-session-note-mode="new">新建后台会话并发送</button>`;
...
function bindNoteDrag(root) {
  ...
  root.addEventListener('pointerup', () => finishNoteDrag(true));
  root.addEventListener('pointercancel', () => finishNoteDrag(false));
...
$('#topbar-note-buttons').onclick = (event) => {
  ...
  if (note) openTopbarNotePanel(note);
};
$('#session-note-buttons').onclick = (event) => {
  ...
  if (note) openSessionNoteMenu(button, note);
};
...
$('#task-groups').onclick = (event) => {
  const board = event.target.closest('[data-board-type]');
  if (board) {
    state.boardType = board.dataset.boardType;
    if (state.status === 'archived') state.status = '';
    applyViewSettings(); renderTaskSidebar(); renderList(); saveLayoutState();
    return;
  }
  const archiveGroup = event.target.closest('[data-archive-group]');
