...
export function createTask({ title, description, color, deadline }) {
  const now = new Date().toISOString();
  const sessionFile = path.join(SESSIONS_DIR, `${randomUUID()}.jsonl`);
  const t = {
    id: randomUUID(),
    ...
    updatedAt: now,
    completedAt: null,
    sessions: [{ id: 'main', title: '主会话', sessionFile: sessionFile, createdAt: now, updatedAt: now }],
  };
  tasks.push(t);
  saveTasks();
  return t;
