...
export const ARCHIVE_RETENTION_MS = 15 * 24 * 60 * 60 * 1000;
...
export function normalizeNotes(input) {
  ...
  for (let index = normalized.length - 1; index >= 0; index--) {
    ...
    const existingPurgeAt = note.purgeAt && !Number.isNaN(new Date(note.purgeAt).getTime()) ? note.purgeAt : null;
    const purgeAt = isArchived
      ? (existingPurgeAt || new Date(new Date(archivedAt).getTime() + ARCHIVE_RETENTION_MS).toISOString())
      : null;
    if (note.purgeAt !== purgeAt) { note.purgeAt = purgeAt; changed = true; }
...
export function createNote({ title, description, color, deadline, pinnedToTopBar, pinnedToSessionBar }) {
  ...
  const note = {
    ...
    archivedAt: null, purgeAt: null, topbarOrder: 0, sessionOrder: 0,
...
export function reorderNotes(placement, ids) {
  ...
  const now = new Date().toISOString();
  [...selected, ...remaining].forEach((note, index) => { note[key] = index + 1; note.updatedAt = now; });
  saveTasks();
