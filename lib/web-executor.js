...
import { EnvHttpProxyAgent, install as installUndici, setGlobalDispatcher } from 'undici';
...
// The CLI configures undici before creating sessions, but this application uses
// the SDK directly. Install the same proxy-aware dispatcher for SDK requests.
// The setting is global in pi, so read it once when the server starts.
function configureHttpProxy() {
  ...
  if (!proxy) {
    ...
    try {
      ...
      proxy = String(settings.httpProxy || '').trim();
    } catch { /* 没有 pi 全局设置 */ }
  }
  if (!proxy) return;
  ...
  try {
    ...
    console.error(`[web-executor] 代理配置失败：${error.message}`);
  }
}
configureHttpProxy();
