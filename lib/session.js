...
export function parseSessionFile(file) {
  ...
  return { exists: true, header, entries: body, leaf, lastMessage, stats };
}
 ...
 *
 * @returns null = 继续执行中；否则 { state: 'review', lastRun: { status, reason, at } }
 */
export function evaluateRunningTask(task, { findPiPids, now = Date.now() }) {
  const runStart = task.lastRun && task.lastRun.at ? new Date(task.lastRun.at).getTime() : 0;
  const parsed = parseSessionFile(task.sessionFile);
  const processAlive = findPiPids(task.sessionFile).length > 0;

  if (!parsed.exists) {
    if (runStart && now - runStart > 180_000) {
      return { state: 'review', lastRun: { status: 'failed', reason: '未生成 session 文件，pi 可能启动失败', at: iso(now) } };
    }
    return null;
  }
  ...
  if (!leafMsg) {
    if (runStart && now - runStart > 180_000) {
      return { state: 'review', lastRun: { status: 'failed', reason: 'pi 未写入任何新消息（可能启动失败）', at: iso(now) } };
    }
    return null;
  }
  ...
  if (m.role === 'assistant') {
    if (m.stopReason === 'stop' || m.stopReason === 'length') {
      return { state: 'review', lastRun: { status: 'done', reason: null, at: iso(now) } };
    }
    if (m.stopReason === 'error') {
      return { state: 'review', lastRun: { status: 'failed', reason: m.errorMessage || '模型返回错误（检查模型连接/认证/限流）', at: iso(now) } };
    }
    if (m.stopReason === 'aborted') {
      return { state: 'review', lastRun: { status: 'terminated', reason: '执行被中断', at: iso(now) } };
    }
    // stopReason === 'toolUse'：工具执行中
    return processAlive
      ? null
      : { state: 'review', lastRun: { status: 'terminated', reason: 'pi 进程已退出（工具执行中）', at: iso(now) } };
  }
  if (m.role === 'user') {
    return processAlive
      ? null
      : { state: 'review', lastRun: { status: 'terminated', reason: 'pi 进程已退出（可能关闭了终端窗口）', at: iso(now) } };
  }
  // toolResult
  return processAlive
    ? null
    : { state: 'review', lastRun: { status: 'terminated', reason: 'pi 执行中进程退出（可能关闭了终端窗口）', at: iso(now) } };
}
